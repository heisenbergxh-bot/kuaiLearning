...
- 题库、术语、参考资料等派生内容同步（工作区、大纲和课程正文已经完成）。
...
2. **继续扩展 API 契约**：工作区、大纲和课程契约已经落地；下一步为测验、术语、参考文档
   和学习记录建立 Pydantic Schema 与 TypeScript API Client。
3. **继续渐进迁移本地数据**：工作区、大纲和课程正文已经同步；继续保留 IndexedDB 作为
   缓存，按领域增加导入/同步，避免直接重写全部前端导致已有功能回退。
